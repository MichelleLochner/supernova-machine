...
def run_ml(X_train, Y_train, X_test, Y_test, **kwargs):
    ...
    setup_plot(ax)
    if SVM:
        legend(('SVM (%.3f)' %(a1), 'Naive Bayes (%.3f)' %(a2), 'KNN (%.3f)' %(a3), 'Random Forest (%.3f)' %(a4)), loc='lower right',  frameon=True, bbox_to_anchor=(0.95, 0.05), fontsize=18)
    else:
        legend(('Naive Bayes (%.3f)' %(a2), 'KNN (%.3f)' %(a3), 'Random Forest (%.3f)' %(a4)), loc='lower right',  frameon=True, bbox_to_anchor=(0.95, 0.05), fontsize=18)
